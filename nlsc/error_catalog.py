...
ERROR_CATALOG: dict[str, ErrorDefinition] = {
    ...
    ELOCK004: ErrorDefinition(
        code=ELOCK004,
        title="Emitter semantics changed",
        summary="`nlsc compile --frozen-lockfile` or `nlsc ci` found a lockfile whose recorded emitter semantics marker no longer matches the current backend.",
        emitted_by=("compile", "ci"),
        common_causes=(
            "The compiler was upgraded and the target emitter now produces different code for the same source.",
            "The lockfile was generated by an older backend semantics revision.",
        ),
        next_steps=(
