...
def check_module(nl_file: NLFile, *, file_token: str = "<source>") -> SemanticCheckResult:
    ...
    result.errors.extend(_check_retry_policies(module, file_token))
...
def _check_retry_policies(module: "IRModule", file_token: str) -> list[Diagnostic]:
    """Checked RETRY:/TIMEOUT: policies (#201).

    Static guarantees only: a bounded budget, retryable identities the
    operation can actually raise, an idempotency key for effectful
    replay, and an explicit timeout outcome.  The checker says nothing
    about the provider actually honoring the key.
    """
    ...
    for operation in module.operations:
        ...
        if operation.retry is not None:
            ...
            if retry.attempts is None or retry.attempts <= 0:
                diagnostics.append(
                    Diagnostic(
                        ...
                        col=None,
                        message=(
                            f"{operation.name}: RETRY has no finite attempt budget"
                        ),
            ...
            # rejected when it provably cannot occur (the failure set is
            # fully known and lacks it).  When the set contains unknown
            # markers the author's classification is the only one
            # available; unlisted unknown failures are not retried.
                ...
                if not unknown_failures and error_type not in known_identities:
                    diagnostics.append(
                        Diagnostic(
                            ...
                            col=None,
                            message=(
                                f"{operation.name}: RETRY lists '{error_type}', which "
                                "is not a proven failure identity of this operation"
                            ),
                            hint=(
                                "Retry only errors from the inferred failure set; "
                                "unknown failures cannot be proven retryable."
                            ),
                        )
                    )
            ...
            if has_effects and retry.idempotency_key is None:
                diagnostics.append(
                    Diagnostic(
                        code=ESEM020,
                        file=file_token,
            ...
            if (
                ...
                diagnostics.append(
                    Diagnostic(
                        ...
                        hint=(
                            "Key on an input so the same request value is used on "
                            "every attempt."
                        ),
                    )
                )
            ...
            if timeout.after_ms is None or timeout.after_ms <= 0:
                diagnostics.append(
                    Diagnostic(
                        ...
                        col=None,
                        message=(
                            f"{operation.name}: TIMEOUT has no positive deadline"
                        ),
                        hint="Declare 'after Nms -> outcome' with N > 0.",
                    )
                )
            elif not timeout.outcome:
                diagnostics.append(
                    Diagnostic(
                        code=ESEM021,
                        file=file_token,
                        ...
                        col=None,
                        message=(
                            f"{operation.name}: TIMEOUT declares no outcome for "
                            "the deadline"
                        ),
                        hint=(
