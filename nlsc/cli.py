...
from .pipeline import SemanticGateResult, evaluate_semantic_gate
from .graph import (
    emit_mermaid,
    emit_dot,
    emit_ascii,
...
def _scaffold_execution_diagnostic(
    source_path: Path, scaffold: set[str], command: str
) -> Diagnostic:
    """Refusal to execute a module whose LOGIC is unresolved (#264).

    A scaffold's prose steps compile to `None` placeholders, so executing it
    exercises code that does not implement the specification — and, before
    this gate, reported success while doing so.
    """
    names = ", ".join(sorted(scaffold))
    return Diagnostic(
        code=ESCAF001,
        ...
        col=None,
        message=(
            f"refusing to {command} an incomplete scaffold: "
            f"{len(scaffold)} ANLU(s) have unresolved executable content ({names})"
        ),
        hint=(
            f"Run `nlsc verify {source_path} --strict` to see every unresolved step, "
            "then rewrite them as executable expressions or implement them with @literal."
        ),
    )


def _refuse_scaffold_execution(
    source_path: Path, gate: SemanticGateResult, command: str, json_output: bool
) -> Optional[int]:
    """Block run/test on a scaffold; returns an exit code when refused."""
    if not gate.scaffold:
        return None
    diagnostic = _scaffold_execution_diagnostic(source_path, gate.scaffold, command)
    if json_output:
        return _emit_json(command, [diagnostic], file=str(source_path))
    _print_semantic_diagnostics(
        f"Error: {_cross()} Incomplete scaffold", [diagnostic], hint=True
    )
    _print_semantic_diagnostics(
        "  Unresolved executable content", gate.scaffold_warnings
    )
    return 1


def _remove_stale_artifact(path: Path) -> None:
    """Delete a previously generated artifact a scaffold has replaced (#264).

    Only files this compiler wrote are removed: a module that regresses into a
    scaffold must not leave its last good build behind to be imported, but a
    hand-written file sharing the name is never touched.
    """
    if not path.is_file():
        return
    try:
        ...
        return
    if "Generated by nlsc" not in head:
        return
    try:
        path.unlink()
    except OSError:
...
def cmd_compile(args: argparse.Namespace) -> int:
    ...
    # Determine output path. #264: a scaffold must never claim the module's
    # importable filename, or it can be imported, packaged, or substituted
    # into a build as if it implemented the specification. Quarantine it as
    ...
    if not json_output:
        if scaffold and not args.output:
            print(
                f"  {_check()} Generated draft {output_path.name} ({line_count} lines)"
                " - incomplete scaffold, not importable as the module"
            )
        else:
    ...
    # A scaffold gets no test artifact (#264): pytest would collect it and
    # report assertion failures against placeholder bodies, which reads as a
    # logic bug rather than an unimplemented spec.
    if test_code and nl_file.tests and not scaffold:
    ...
    if frozen:
        ...
        if existing_lock is None:
            ...
            print(f"Hint: {diagnostic.hint}", file=sys.stderr)
            return 1
        stale = verify_lockfile(existing_lock, nl_file)
        if stale:
            diagnostics = lockfile_outdated_diagnostics(source_path, nl_file, stale)
            if json_output:
                return _emit_json("compile", diagnostics, file=str(source_path))
            print("Error: lockfile is not current:", file=sys.stderr)
            for diagnostic in diagnostics:
                print(f"  - {diagnostic.message}", file=sys.stderr)
            print(
                "Hint: Run `nlsc compile <file>` without --frozen-lockfile once "
        ...
        if target_lock is not None and _hash_content(generated_code) != target_lock.hash:
            diagnostic = Diagnostic(
                ...
                col=None,
                message=(
                    f"compiled output does not reproduce the locked target "
                    f"hash for target '{target}'"
                ),
                hint=(
                    "The lockfile was generated by a different compiler "
                    "version; regenerate it and commit it."
                ),
            )
            if json_output:
                return _emit_json("compile", [diagnostic], file=str(source_path))
            ...
            return 1
        if not json_output:
            print(f"  {_check()} Lockfile verified (frozen; not rewritten)")
    else:
...
def cmd_test(args: argparse.Namespace) -> int:
    ...
    # #264: never execute a scaffold. Its prose steps compile to `None`
    # placeholders, so a test run reports assertion failures that look like
    # logic bugs in code that was never implemented.
    scaffold_exit = _refuse_scaffold_execution(source_path, gate, "test", json_output)
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    # #264: never execute a scaffold. Before this gate `nlsc run` executed the
    # placeholder body, printed its `None` result, and exited 0.
    scaffold_exit = _refuse_scaffold_execution(source_path, gate, "run", json_output)
    if scaffold_exit is not None:
        return scaffold_exit
...
def cmd_ci(args: argparse.Namespace) -> int:
    """Automation-native gate (#149): strict semantics + frozen lockfile.

    Stages: parse -> strict semantic gate -> lockfile freshness -> optional
    ...
    def _fail(stage: str, new_diagnostics: list[Diagnostic]) -> int:
        stages[stage] = "failed"
        if json_output:
            return _emit_json(
                "ci", new_diagnostics, file=str(source_path), stages=stages
            )
        print(f"ci: {stage} stage failed:", file=sys.stderr)
    ...
    if not source_path.exists():
        return _fail("parse", [missing_file_diagnostic(source_path)])

    try:
        nl_file = parse_nl_file_auto(source_path)
    except ParseError as e:
        return _fail("parse", [parse_error_diagnostic(source_path, e)])
    stages["parse"] = "passed"

    # Strict gate: fatal tiers, contract drift, and scaffold content all
    # fail; CI never accepts unresolved executable content.
    gate = evaluate_semantic_gate(nl_file, file_token=str(source_path))
    blocking = gate.fatal + gate.strict_only + gate.scaffold_warnings
    if blocking:
    ...
    if existing_lock is None:
        return _fail(
            "lockfile", [lockfile_unavailable_diagnostic(lock_path)]
        )
    stale = verify_lockfile(existing_lock, nl_file)
    if stale:
        return _fail(
    ...
    if getattr(args, "compile", False):
        ...
        try:
            ...
            return _fail("compile", [diagnostic])

        # Reproducibility: the regenerated output must reproduce the
        # hash recorded in the current lockfile.
        target_lock = existing_lock.targets.get(target)
        if target_lock is not None and hash_content(generated_code) != target_lock.hash:
            diagnostic = Diagnostic(
                code=ELOCK002,
                file=str(source_path),
                line=None,
                ...
                hint=(
                    "The lockfile was generated by a different compiler "
                    "version; regenerate it with `nlsc compile` and commit it."
                ),
            )
            return _fail("compile", [diagnostic])
        ...
        try:
            ...
            diagnostic = artifact_io_diagnostic(
                output_path, exc, action="write", command="ci"
            )
            return _fail("compile", [diagnostic])
        if test_code and nl_file.tests:
            ...
            try:
                ...
                diagnostic = artifact_io_diagnostic(
                    test_path, exc, action="write", command="ci"
                )
                return _fail("compile", [diagnostic])
        stages["compile"] = "reproducible"
    ...
    if getattr(args, "test", False):
        ...
        test_code_exit = cmd_test(test_args)
        if test_code_exit != 0:
            stages["test"] = "failed"
            return test_code_exit if json_output else 1
        stages["test"] = "passed"

    if json_output:
        return _emit_json(
            "ci", [], file=str(source_path), stages=stages, target=target
        )
    stage_summary = ", ".join(f"{name}: {value}" for name, value in stages.items())
    print(f"ci passed ({stage_summary})")
    return 0
...
""",
    ...
    compile_parser.add_argument(
        "--frozen-lockfile",
        action="store_true",
        help="Require a current lockfile; never rewrite it; verify output hash",
    )
    ...
    ci_parser = subparsers.add_parser(
        "ci", help="CI gate: strict semantics + frozen lockfile (+ optional compile/test)"
    )
    ci_parser.add_argument("file", help="Input .nl file")
    ...
    ci_parser.add_argument(
        "--compile",
        action="store_true",
        help="Also compile, verifying the output reproduces the locked hash",
    )
    ci_parser.add_argument(
        "--test", action="store_true", help="Also run @test specifications"
    )
    ci_parser.add_argument(
        "-t", "--target", help="Target language override (default: @target)"
    )
    ci_parser.add_argument(
