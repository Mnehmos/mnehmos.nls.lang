...
class TestIsolationSemantics:
    ...
    def test_function_calls_are_independent(self, python_runner: PythonRunner):
...
"""
        ...
        result1 = python_runner.execute(code, "pure_double", (5,))
        result2 = python_runner.execute(code, "pure_double", (5,))
        result3 = python_runner.execute(code, "pure_double", (10,))
    ...
    def test_instances_are_independent(self, python_runner: PythonRunner):
...
"""
        ...
        result1 = python_runner.create_instance(code, "Counter", {"value": 10})
        result2 = python_runner.create_instance(code, "Counter", {"value": 20})
    ...
    def test_compiled_code_reusable(self, python_runner: PythonRunner):
...
"""
        ...
        results = [
            python_runner.execute(code, "add", (1, 2)),
            python_runner.execute(code, "add", (3, 4)),
            python_runner.execute(code, "add", (1, 2)),  # Same as first
            python_runner.execute(code, "add", (0, 0)),
        ]
    ...
    def test_guard_failures_dont_affect_later_calls(self, python_runner: PythonRunner):
...
"""
        ...
        # First call fails
        result1 = python_runner.execute(code, "safe_divide", (10, 0))
        assert not result1.success

        # Second call should work fine
        result2 = python_runner.execute(code, "safe_divide", (10, 2))
        assert result2.success
        assert result2.return_value == 5

        # Third call with same failing input
        result3 = python_runner.execute(code, "safe_divide", (5, 0))
        assert not result3.success

        # Fourth call should still work
        result4 = python_runner.execute(code, "safe_divide", (20, 4))
        assert result4.success
        assert result4.return_value == 5

    def test_separate_namespaces(self, python_runner: PythonRunner):
...
"""
        ...
        result1 = python_runner.execute(code, "identity", (42,))
        result2 = python_runner.execute(code, "identity", (99,))
